...
type Config struct {
	...
	// User session timeout in seconds (inactivity period after which a user is removed)
	SessionTimeoutSeconds int `json:"sessionTimeoutSeconds"`
	
	// Message to display when the queue is full
	QueueFullMessage string `json:"queueFullMessage"`
	
	// When true, use X-Forwarded-For header for IP detection
